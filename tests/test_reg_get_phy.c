...
static int test_reg_read_basic(void) {
  static const struct reg_field fields[] = {{"bit1", 0, 0, 1, REG_VOLATILE},
  ...
  if (val_get != val_read) {
    TEST_FAIL("reg_get bit64 returned 0x%llx, expected same as reg_get 0x%llx",
              (unsigned long long)val_get, (unsigned long long)val_read);
    return -1;
  }

  // verify that data[] was updated with the mock reads
  if (data[0] != 0xFFFFFFFF || data[1] != 0xFFFFFFFF || data[2] != 0x00000000) {
...
static int test_reg_read_cross_boundary(void) {
  static const struct reg_field fields[] = {
      {"cross17", 0, 31, 17, REG_VOLATILE}, {NULL, 0, 0, 0, REG_VOLATILE}};
  ...
  if (data[0] != 0x80000000 || data[1] != 0x00000001 || data[2] != 0x00000000) {
    TEST_FAIL("reg_get did not update device data correctly on cross boundary");
    return -1;
  }
...
static int test_reg_read_invalid_field(void) {
  static const struct reg_field fields[] = {{"bit4", 0, 0, 4, REG_VOLATILE},
                                            {NULL, 0, 0, 0, REG_VOLATILE}};
