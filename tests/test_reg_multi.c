...
static const struct reg_field test_dev_map[] = {
    ...
    {"POWERDOWN", 0, 0, 1, 0},     {"RESET", 0, 1, 1, 0},
    {"MUXOUT_LD_SEL", 0, 2, 1, 0}, {"FCAL_EN", 0, 3, 1, 0},
    {"R0_RES1", 0, 4, 1, 0},       {"FCAL_LPFD_ADJ", 0, 5, 2, 0},
    {"FCAL_HPFD_ADJ", 0, 7, 2, 0}, {"OUT_MUTE", 0, 9, 1, 0},
    {"R0_RES2", 0, 10, 1, 0},      {"ADD_HOLD", 0, 11, 1, 0},
    {"R0_RES3", 0, 12, 2, 0},      {"VCO_PHASE_SYNC_EN", 0, 14, 1, 0},
    {"RAMP_EN", 0, 15, 1, 0},      {"R34_RES", 34, 3, 13, 0},
    {"PLL_N_MSB", 34, 0, 3, 0},    {"PLL_N_LSB", 36, 0, 16, 0},
    {"R37_RES2", 37, 0, 8, 0},     {"PFD_DLY_SEL", 37, 8, 6, 0},
    {"R37_RES1", 37, 14, 1, 0},    {"MASH_SEED_EN", 37, 15, 1, 0},
    {"PLL_NUM", 43, 0, 32, 0},     {NULL, 0, 0, 0, 0} // sentinel
};
...
static int test_rw32(int ch, const uint32_t pattern) {
  ...
  if (phi42 != msb) {
    TEST_FAIL("MSB written as 0x%" PRIx32 ", should be 0x%" PRIx32, phi42, msb);
    return -1;
  }

  const uint32_t phi43 = dummy_phys_reg[1];
  if (phi43 != lsb) {
    TEST_FAIL("LSB written as 0x%" PRIx32 ", should be 0x%" PRIx32, phi43, lsb);
    return -1;
  }
