...
#include <string.h> /* For strcmp(3). */
    ...
    defined(__NeXT__) || defined(__bsd__)
#define OS_BSD 1
...
// #include <stdio.h> // JK edit 8may24    /* For NULL, size_t, vsnprintf(3),
// and vasprintf(3). */
#ifdef VA_START
#undef VA_START
...
static void fmtstr(char *str, size_t *len, size_t size, const char *value,
  ...
  for (strln = 0; (noprecision || strln < precision) && value[strln] != '\0';
       strln++)
    continue;
...
static void fmtflt(char *str, size_t *len, size_t size, LDOUBLE fvalue,
...
static int convert(UINTMAX_T value, char *buf, size_t size, int base,
                   int caps) {
...
#endif /* !OS_BSD */
                        0};
  const char *long_fmt[] = {"foo|%0123ld|bar",
#if !OS_IRIX
                            "% '0123ld",
                            "%+'0123ld",
                            "%-'123ld",
...
#endif /* !OS_IRiX */
      "%123.9lu", "% 123.9lu", "%+123.9lu", "%-123.9lu", "%0123lu", "% 0123lu",
      "%+0123lu", "%-0123lu", "%5.5lu", "%+22.33lu", "%01.3lu", "%1.5lu",
      "%-1.5lu", "%44lu", "%lu",
      /* "%o" formats. */
      "foo|%#0123lo|bar", "%#123.9lo", "%# 123.9lo", "%#+123.9lo", "%#-123.9lo",
      "%#0123lo", "%# 0123lo", "%#+0123lo", "%#-0123lo", "%#5.5lo",
      "%#+22.33lo", "%#01.3lo", "%#1.5lo", "%#-1.5lo", "%#44lo", "%#lo",
      "%123.9lo", "% 123.9lo", "%+123.9lo", "%-123.9lo", "%0123lo", "% 0123lo",
      "%+0123lo", "%-0123lo", "%5.5lo", "%+22.33lo", "%01.3lo", "%1.5lo",
      "%-1.5lo", "%44lo", "%lo",
      /* "%X" and "%x" formats. */
      "foo|%#0123lX|bar", "%#123.9lx", "%# 123.9lx", "%#+123.9lx", "%#-123.9lx",
      "%#0123lx", "%# 0123lx", "%#+0123lx", "%#-0123lx", "%#5.5lx",
      "%#+22.33lx", "%#01.3lx", "%#1.5lx", "%#-1.5lx", "%#44lx", "%#lx", "%#lX",
      "%123.9lx", "% 123.9lx", "%+123.9lx", "%-123.9lx", "%0123lx", "% 0123lx",
      "%+0123lx", "%-0123lx", "%5.5lx", "%+22.33lx", "%01.3lx", "%1.5lx",
      "%-1.5lx", "%44lx", "%lx", "%lX", NULL};
  unsigned long int ulong_val[] = {
#ifdef ULONG_MAX
      ULONG_MAX,
#endif /* ULONG_MAX */
      91340,     341, 134, 0203, 1, 0};
  const char *llong_fmt[] = {"foo|%0123lld|bar",
                             "%123.9lld",
                             "% 123.9lld",
                             "%+123.9lld",
                             "%-123.9lld",
...
#endif /* LLONG_MIN */
      -91340,    91340, 341, 134, 0203, -1, 1, 0};
  const char *string_fmt[] = {"foo|%10.10s|bar",
                              "%-10.10s",
                              "%10.10s",
                              "%10.5s",
                              "%5.10s",
