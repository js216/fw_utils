 ...
 */
static inline size_t reg_min(size_t a, size_t b) { return (a < b) ? a : b; }
 ...
 */
static inline size_t reg_cdiv(size_t x, size_t y) { return (x + y - 1) / y; }
...
uint32_t reg_read(const struct reg_dev *d, const size_t reg) {
  ...
  if (reg >= d->reg_num) {
    ERROR("register outside device boudns");
    return 0;
  }
